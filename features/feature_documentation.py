group_doc = {
      ...
      "Haralick features":
        "The Haralick features aim at characterizing the texture of objects by means of joint"
        "distribution of pixel value combinations."
        "In histograms, one tries to analyze the frequency of certain grey level values in an"
        "image. The inconvenience of this representation is that the spatial distribution of these"
        "values is completely lost. One method to address this inconvenience is to record combinations"
        "of pixel values at a certain distance. This can be done by the cooccurrence"
        "matrix depending on distance d and angle \(\Phi\)",   
...
feature_doc = {
    "convexhull___average-clump-dispacement":
      " ",
